...
class GeneticAlgorithm:
    ...
    def __init__(self, agent_descriptor=None, ga_descriptor=None):
        ...
        logging.info(' Agent Initialization '.center(120, '-'))
    ...
    def evolve(self):
        ...
        for generation in self.generations:
            ...
            logging.info(" Generation %s ".center(120, '-') % str(generation.id))
        ...
        visual = Visualization(self.agent_descriptor, self.generations, self.ga_descriptor.config['selection']['truncation']['elite_size'])
    ...
    def fitness(self, generation):
        ...
        print (" Generation %s ".center(120, '-') % str(generation.id))

        results = [self.pool.apply_async(argos, args=(self.agent_descriptor.config['argos_xml'][:-4] + '_' + str(number) + self.agent_descriptor.config['argos_xml'][-4:], agent, obs_agent)) for number, agent, obs_agent in zip(list(xrange(40)), self.agents, self.obs_agents)]
